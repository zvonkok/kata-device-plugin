...
- **Supply, not demand** — the plugin declares what exists; how it is consumed is the scheduling layer's decision. Whole node vs subset (`nvidia.com/gpu: 4` vs `: 2`), exclusive tenancy, tray semantics: all of that is expressed in pod specs, node labels, and taints — never encoded here. No aggregate resources, no consumption modes (see ARCHITECTURE.md).
